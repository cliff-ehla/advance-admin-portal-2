...
export const eventContent = (arg) => {
	...
	if (props.type === 'book') {
		...
		} else if (props.remark) {
			alert_el.innerHTML = '<svg class="w-4 fill-current" viewBox="0 0 36 36" class="svgicon center-block replaced-svg"><path d="M33.188 1.356l1.455 1.456c1.81 1.81 1.81 4.74 0 6.55l-2.547 2.546-8.006-8.004 2.55-2.547c1.805-1.81 4.74-1.81 6.548 0zM10.99 33.016l-8.006-8.008L22.634 5.36l8.006 8.005-19.65 19.65zM2.258 27.92l5.822 5.822L0 36l2.258-8.08z"></path></svg>'
		} else if (props.override_salary) {
		...
		} else if (props.trial_success_buy_time) {
	...
	} else if (props.type === 'trial_option') {
		let title_el = document.createElement('div')
		title_el.innerText = ' Option'
		let time_el = genTimeEl(arg)
		wrapper.appendChild(time_el)
		wrapper.appendChild(title_el)
	} else if (props.type === 'trial_option_temp') {
		let title_el = document.createElement('div')
		title_el.innerText = '[tbc] opt'
		let time_el = genTimeEl(arg)
		wrapper.appendChild(time_el)
		wrapper.appendChild(title_el)
	} else if (props.type === 'edit_temp') {
		let title_el = document.createElement('div')
		...
		let title_el = document.createElement('div')
		title_el.innerText = getStudentTitle(props)
		let time_el = genTimeEl(arg)
		wrapper.appendChild(time_el)
		wrapper.appendChild(title_el)
	} else if (props.type === 'ff_reserved') {
		let title_el = document.createElement('div')
		title_el.innerText = getStudentTitle(props) || 'Option FF'
		let time_el = genTimeEl(arg)
		wrapper.appendChild(time_el)
		wrapper.appendChild(title_el)

...
<div class="flex mb-4">
	<div>
		...
		<p>{student.nickname || 'no student selected'}</p>
	</div>
...
<div class="overflow-y-scroll h-96 my-4 border border-gray-300 rounded">
	{#each $student_store as s}
		<div class:text-blue-500={student.user_id === s.user_id} class="cursor-pointer hover:bg-blue-50 px-4 py-2 border-b border-gray-200"
		     on:click={() => {student = s}}>
			<div class="flex items-center">
				<p class="font-bold">{s.nickname || '(no name)'}</p>
				<p class="bg-gray-200 border-gray-300 border rounded-sm text-xs ml-2 px-1">{s.level}</p>
			</div>
			<p class="text-sm text-gray-500">@{s.username} <span class="ml-2">ID:{s.user_id}</span></p>
		</div>
	{/each}
</div>
...
<div class="flex justify-between">
	...
	<Button disabled={!student.user_id} button_class="flex-1 py-2 ml-4" on:click={onConfirmClick}>確定</Button>
</div>